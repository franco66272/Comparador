...
def normalizar_fecha(v):
    if isinstance(v, (int, float)) and v > 1_000_000_000:
        try:
            return datetime.fromtimestamp(float(v) / (1000 if v > 10_000_000_000 else 1)).strftime("%Y-%m-%dT%H:%M:%S")
        except Exception:
            pass
    s = str(v).strip()
    if s.isdigit() and len(s) >= 10:
        try:
            return datetime.fromtimestamp(int(s) / (1000 if len(s) >= 13 else 1)).strftime("%Y-%m-%dT%H:%M:%S")
        except Exception:
            pass
    return s
    ...
    if isinstance(obj, dict):
        for v in obj.values():
            if isinstance(v, (dict, list)):
                recorrer(v, encontrados, depth + 1)
...
def extraer_texto(texto):
    ...
    if not texto:
        return encontrados
    try:
        encontrados.extend(recorrer(json.loads(texto)))
    except Exception:
        pass
    return encontrados
...
def extract_http(url, session):
    ...
    for script in soup.find_all("script"):
        ...
        if txt and len(txt) < 12_000_000:
            candidates.extend(extraer_texto(txt))
...
def extract_browser(url, browser, sink):
    ...
    def on_response(resp):
        try:
            ...
            low = resp.url.lower()
            interesting = "json" in ct or "javascript" in ct or any(k in low for k in ("api", "graphql", "ajax", "chart", "price", "history", "data", "product"))
            if not interesting:
                return
            body = resp.text()
    ...
    try:
        ...
        for txt in page.locator("script").all_text_contents():
            if txt and len(txt) < 12_000_000:
                candidates.extend(extraer_texto(txt))
        try:
            sink["storage"] = page.evaluate("() => ({local:{...localStorage},session:{...sessionStorage}})")
        except Exception:
            pass
...
def main():
    ...
    try:
        history = json.loads(HISTORIAL.read_text(encoding="utf-8")) if HISTORIAL.exists() else {}
    except Exception:
        history = {}
    objectives = [p for p in catalog if str(p.get("tienda", "")).strip().lower() == "venex" and url_hardgamers(p)]
    try:
        limit = int(os.environ.get("HG_MAX_PRODUCTOS", "0") or "0")
    except ValueError:
        limit = 0
    if limit > 0:
    ...
    results = {
        ...
        "id_directo": len(objectives),
        "match": len(objectives),
        "series": 0,
        "puntos": 0,
        "fallback_browser": 0,
        "errores": [],
        "modo": "direct-id-http-playwright-v6",
    }
    debug = {"productos": []}
    ...
    with sync_playwright() as pw:
        ...
        for i, product in enumerate(objectives, 1):
            url = url_hardgamers(product)
            key = clave_producto(product)
            sink = {"producto": product.get("nombre"), "url": url}
            try:
                series, http_meta = extract_http(url, session)
                sink["http"] = http_meta
                if len(series) < 2:
                ...
                if len(series) >= 2:
                    history[key] = series
                    results["series"] += 1
                    results["puntos"] += len(series)
                    sink["serie_puntos"] = len(series)
                else:
                    results["errores"].append({"producto": product.get("nombre"), "url": url, "error": "sin_serie"})
            except Exception as exc:
                results["errores"].append({"producto": product.get("nombre"), "url": url, "error": f"{type(exc).__name__}: {exc}"})
            if i <= MAX_DEBUG:
                debug["productos"].append(sink)
            if i % 25 == 0 or i == len(objectives):
                print(f"[{i}/{len(objectives)}] ids={results['id_directo']} series={results['series']} puntos={results['puntos']} browser={results['fallback_browser']}")
            time.sleep(PAUSA)
        browser.close()
