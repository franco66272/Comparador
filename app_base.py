...
def extraer_detalle_en_vivo(url):
    ...
    resultado["ok"]=bool(resultado["precio"] or resultado["descripcion"] or resultado["imagen"]); return resultado
...
def inicio():
    ...
    tiendas=sorted(set(p.get("tienda") for p in productos if p.get("tienda"))); conteo_tiendas={}
    for p in productos:
        tienda=p.get("tienda")
        if tienda: conteo_tiendas[tienda]=conteo_tiendas.get(tienda,0)+1
    tiendas_info=[{"clave":t,"nombre":nombre_tienda_visible(t),"productos":conteo_tiendas[t]} for t in tiendas]; tiendas_info.sort(key=lambda x:(-x["productos"],x["nombre"].lower())); tiendas_principales,tiendas_restantes=tiendas_info[:8],tiendas_info[8:]
    categorias=construir_categorias(productos); candidatas=[]
    for p in productos:
        if not p.get("nombre") or not p.get("precio"): continue
        score=(40 if p.get("stock") else 0)+(30 if imagen_utilizable(p.get("imagen")) else 0)+(10 if p.get("url") else 0)+(5 if len(str(p.get("nombre")))<=100 else 0); candidatas.append((score,nombre_tienda_visible(p.get("tienda")),str(p.get("nombre")).lower(),p))
...
@app.route("/health")
def health(): return {"ok":True,"servicio":"TecnoRadar","productos":len(filtrar_tiendas_publicables(cargar_productos()))}
...
def resolver_imagen():
    ...
    if producto_url:
        clave=producto_url; encontrada=_CACHE_IMAGENES.get(clave)
        if clave not in _CACHE_IMAGENES:
            encontrada=None
            try:
                r=requests.get(producto_url,headers=IMAGEN_HEADERS,timeout=7)
                if r.status_code==200:
                    soup=BeautifulSoup(r.text,"html.parser"); meta=soup.select_one('meta[property="og:image"]'); encontrada=_imagen_candidata(meta.get("content") if meta else None,r.url)
            except requests.RequestException: encontrada=None
            _CACHE_IMAGENES[clave]=encontrada
        if encontrada: return redirect(encontrada,code=302)
    return ("",404)
@app.route("/salud")
def salud(): return {"estado":"OK","tiendas":cargar_salud_tiendas()}
